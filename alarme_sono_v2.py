...
# =========================
# CAMINHOS DO PROJETO (REFATORAÇÃO)
# =========================

# caminho base do projeto (onde está este arquivo .py)
...
ALARM_SOUND_PATH = Path(
    os.getenv(
        "ALARM_SOUND_PATH",
        BASE_DIR / "Alarme" / "alarme.mp3"
    )
)

# caminho para o modelo de landmarks do dlib
PREDICTOR_PATH = Path(
    os.getenv(
        "PREDICTOR_PATH",
...
if not ALARM_SOUND_PATH.exists():
    raise FileNotFoundError(f"Arquivo de áudio não encontrado: {ALARM_SOUND_PATH}")

if not PREDICTOR_PATH.exists():
    raise FileNotFoundError(
        f"Modelo shape_predictor não encontrado: {PREDICTOR_PATH}"
    )
...
while True:
    ...
    frame = imutils.resize(
        frame,
        width=800
    )  # redimensiona o frame para uma largura de 800 pixels (você pode aumentar o tamanho da tela aqui, mas isso pode ser custoso dependendo do computador), mantendo a proporção.
    ...
    rects = detector(
        gray,
        0
    )  # roda o detector de faces do Dlib em escala de cinza.

    for rect in rects:  # itera sobre cada retângulo de rosto detectado.
        shape = predictor(
            gray,
            rect
        )  # busca os pontos chaves do rosto
        ...
        ear = (
            eye_aspect_ratio(leftEye)
            + eye_aspect_ratio(rightEye)
        ) / 2.0  # calcula a média do ear dos dois olhos

        mar = mouth_aspect_ratio(mouth)

        nep = (
            normalized_eyebrow_position(leftEyebrow, leftEye)
            + normalized_eyebrow_position(rightEyebrow, rightEye)
        ) / 2.0  # calcula a média do nep
        ...
        cv2.putText(frame, f"EAR: {ear:.2f}", (600, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
        cv2.putText(frame, f"MAR: {mar:.2f}", (600, 60),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
        cv2.putText(frame, f"NEP: {nep:.2f}", (600, 90),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)

    cv2.imshow(
        "Detector de Sono ao Volante",
        frame
    )  # Mostra a janela com o frame final
